...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train=False, skip_test=False, wandb=None, tb_writer=None, dataset_name=None, logger=None):
    with torch.no_grad():
        ...
        from render import render_video, render_set_for_FPS_test
        # test fps and render video
        args.n_frames = 300
        render_video(args, dataset.model_path, "train", scene.loaded_iter, scene.getTrainCameras(), gaussians, pipeline, background, show_level=False, ape_code=-1)
        render_set_for_FPS_test(dataset.model_path, "train", scene.loaded_iter, scene.getTrainCameras(), gaussians, pipeline, background, show_level=False, ape_code=-1)
