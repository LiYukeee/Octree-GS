...
def readCamerasFromTransforms(path, transformsfile, random_background, white_background, extension=".png", undistorted=False, is_debug=False):
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in enumerate(frames):
            ...
            # temp add 'input' for Matrix dataset  添加一个input
            dirname = os.path.dirname(cam_name)
            filename = os.path.basename(cam_name)
            cam_name = os.path.join(dirname, "input", filename)
...
def readColmapSceneInfo(path, images, eval, ds, llffhold=8):
    ...
    # ply_path = os.path.join(path, "sparse/0/sparse.ply")
